<template>
    <Head>
        <title>Edit Addon - Rental Mobil</title>
    </Head>
...
export default {
    ...
    setup(_props) {
        const form = ref({
            name: _props.addon.name,
            price: _props.addon.price,
            type: _props.addon.type,
        });

        const submitForm = () => {
            router.put(`/admin/addons/${_props.addon.id}`, form.value, {
                onSuccess: () => {
                    Swal.fire("Berhasil!", "Addon berhasil diperbarui.", "success").then(() => {
